...
let fileStr = `@import '../src/assets/variables.scss';\n`;
let tasks = [];
config.nav.map(item => {
    item.packages.forEach(element => {
        ...
        tasks.push(
            fs
                ...
                .then(success => {
                    fileStr += `@import '../src/packages/${folderName}/index.scss';\n`;
                })
                .catch(error => {})
        );
    });
...
tasks.push(fs.copy(path.resolve(__dirname, '../src/assets/styles'), path.resolve(__dirname, '../dist/styles')));
